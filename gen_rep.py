...
class GenRepHighlight(object):
    ...
    def run(self, matchArg=None, startRegionsArg=None):
        ...
        if startRegions and match:
            ...
            view.add_regions("GenRep", positiveMatch, "comments", "", sublime.DRAW_OUTLINED)
        else:
            view.erase_regions("GenRep")


class GenRepCommand(object):
    """
    Highlights regions or regular expression matches.
    """
...
class GenRepPromptCommand(sublime_plugin.WindowCommand):
    """
    ...
    def highlighter(self, regex=None):
        highlighter = GenRepHighlight(view=self.view())
        if regex:
    ...
    def replace(self, replace):
        ...
        GenRepIt = GenRepCommand(view=self.view())
        GenRepIt.run(find=self.findStr, replace=self.replaceStr, startRegionsArg=self.starterRegions)
